...
type MD5SaltPasswordEncoder struct{
	SaltLen int
	Iterations int
	KeyLen int
}

func (m *MD5SaltPasswordEncoder) EncodePassword(rawPassword string) string {
...
func (m *MD5SaltPasswordEncoder) Verify(rawPassword, encodedPassword string) bool {
	options := &password.Options{m.SaltLen, m.Iterations, m.KeyLen, sha512.New}
